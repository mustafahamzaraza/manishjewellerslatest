...
class CartController extends ChangeNotifier {
  ...
  //
  //     // Log the API URL
  //     const apiUrl = AppConstants.getCartDataUri;
  //     debugPrint("Fetching cart data from API: $apiUrl");
  //
  ...
  //       _cartList = [];
  //       apiResponse.response!.data.forEach((cart) {
  //         _cartList.add(CartModel.fromJson(cart));
  //       });
  //
  //       // Log the cart data fetched from the API
  //       debugPrint("Cart data fetched from API: ${_cartList.toString()}");
  //
  //       // Optionally, update local storage with the new API data
  ...
  //       _cartLoading = false;
  //     } else {
  //       // If response is null or the status code is not 200, handle the error
  //       _cartLoading = false;
  //       debugPrint("Error fetching data: ${apiResponse.error}");
  //
  //       ApiChecker.checkApi(apiResponse);  // Handle the error accordingly (like showing a message)
  //     }
  //
  //     notifyListeners();
  //     return apiResponse;
  //   } else {
  //     // If reload is false, return cached data or handle accordingly
  //     return ApiResponse.withSuccess(Response(
  //       requestOptions: RequestOptions(path: AppConstants.getCartDataUri),
  //       statusCode: 200,
  //       data: _cartList,  // Here, _cartList can be the data you want to return
  //     ));
  //   }
  // }
  ...
  Future<ApiResponse> getCartData(BuildContext context, {bool reload = true}) async {
    ...
    if(localData != null) {
      ...
      // Log the API URL
      const apiUrl = AppConstants.getCartDataUri; // Assuming this is our API URL
      debugPrint("Fetching cart data from API: $apiUrl");

      // Log the cart data from local storage
      debugPrint("Cart data from local storage: ${_cartList.toString()}");
  ...
  Future<ApiResponse> addToCartAPI(CartModelBody cart, BuildContext context, List<ChoiceOptions> choices, List<int>? variationIndexes, {int buyNow = 0, int? shippingMethodExist, int? shippingMethodId}) async {
    ...
    const apiUrl = AppConstants.addToCartUri;
    ...
    var body = json.encode({
      ...
      "digital_variant_price": cart.digitalVariantPrice,
      "choices": "", // Assuming 'ChoiceOptions' has a 'toJson' method
      "variation_indexes": variationIndexes,
      "buy_now": buyNow,
      "shipping_method_exist": shippingMethodExist,
      "shipping_method_id": shippingMethodId,
    });

    // Print the request body only
    debugPrint("Request Body: $body");
    debugPrint("Url: $apiUrl");

    // Manually print the properties of CartModelBody
    print("Cart Data:");
    print("Product ID: ${cart.productId}");
    print("Variant: ${cart.variant}");
    print("Color: ${cart.color}");
    print("Variation: ${cart.variation}");
    print("Quantity: ${cart.quantity}");
    print("Variant Key: ${cart.variantKey}");
    print("Digital Variant Price: ${cart.digitalVariantPrice}");
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      debugPrint("API URL add to cart: $apiUrl");
      debugPrint("Successfully added to cart: ${apiResponse.response!.data}");
