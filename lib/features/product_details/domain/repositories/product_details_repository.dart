...
class ProductDetailsRepository implements ProductDetailsRepositoryInterface {
  ...
  Future<ApiResponse> get(String productID) async {
    try {
      final url = '${AppConstants.productDetailsUri}$productID?guest_id=1';
  ...
  // New `getWithCarat` method
  @override
  Future<ApiResponse> getWithCarat(String productID, String carat) async {
    try {
      final url = '${AppConstants.productDetailsUri}$productID?guest_id=1&type=$carat';
      final response = await dioClient!.get(url);
      return ApiResponse.withSuccess(response);
  ...
  Future<ApiResponse> getCount(String productID) async {
    try {
      final response = await dioClient!.get(
          AppConstants.counterUri + productID);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> getSharableLink(String productID) async {
    try {
      final response = await dioClient!.get(
          AppConstants.socialLinkUri + productID);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
