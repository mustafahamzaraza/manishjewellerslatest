...
class _ProductDetailsState extends State<ProductDetails> {
  ...
  //
  List<String> caratOptions = ["18caret", "22caret", "24caret"];
  String selectedCarat = "22caret";
  ...
  _loadData( BuildContext context) async{
    Provider.of<ProductDetailsController>(context, listen: false).getProductDetails(context, widget.productId.toString(), widget.slug.toString(),selectedCarat);
  ...
  void initState() {
    ...
    _loadData(context);
    fetchProductDetails(selectedCarat);
    super.initState();
  }

  Future<void> fetchProductDetails(String carat) async {
    try {
      // Fetch product details using the selected carat
      var response = await Provider.of<ProductDetailsController>(context, listen: false)
          .getProductDetails(context, widget.productId.toString(), widget.slug.toString(), carat);

      print("Product Carat Response data: ${response.toString()}  & ${carat} ${selectedCarat}");
      print("Price: ${response['unit_price']}");
      print("carat: ${carat} selected carat ${selectedCarat}");

      setState(() {
        productDetails = response; // Save response (e.g., price) to update the UI
      });
    } catch (e) {
            ...
            builder: (context, details, child) {
              ...
              return SingleChildScrollView(
                ...
                Column(children: [
                 ...
                 //Text('${details.productDetailsModel?.unitPrice}'),
                  ...
                  //       color: Colors.lightBlueAccent, // Gold color for the header
                  //     ),
                  //   ),
                  // ),
                  // SizedBox(height: 10),

                  // Dropdown button with improved styling for Gold theme
                  // Padding(
                  ...
                  //       value: selectedCarat,
                  //       isExpanded: true,
                  //       icon: Icon(Icons.arrow_drop_down, color: Colors.lightBlueAccent), // Gold color for the dropdown arrow
                  //       onChanged: (String? newCarat) {
                  //         setState(() {
                  //           selectedCarat = newCarat!;
                  //         });
                  //         fetchProductDetails(newCarat!);
                  //       },
                  //       items: caratOptions.map<DropdownMenuItem<String>>((String value) {
                  ...
                  //             style: TextStyle(
                  //               fontSize: 16,
                  //               color: Colors.lightBlueAccent, // Gold color for the dropdown items
                  //             ),
                  //           ),
                  //         );
                  //       }).toList(),
                  ...
                  // SizedBox(height: 20),
                  //
                  // // Display the product price with gold and premium look
                  ...
                  //           fontSize: 20,
                  //           fontWeight: FontWeight.bold,
                  //           color: Colors.amber, // Gold color for the "Price" text
                  //         ),
                  //       ),
                  //       SizedBox(height: 5),
                  //       Text(
                  //         // "\$₹${productDetails['unit_price']}",
                  //         "₹${productDetails.unitPrice.toStringAsFixed(2)}"
                  //         ,style: TextStyle(
                  //           fontSize: 24,
                  //           fontWeight: FontWeight.w600,
                  //           color: Colors.green[700], // A deep green color for the price to contrast with gold
                  //         ),
                  //       ),
                  //     ],
                  //   ) else Text("Price: ₹ ${details.productDetailsModel?.unitPrice}"),
