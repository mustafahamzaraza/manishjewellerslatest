...
//         });
        ...
        //
        // // Retrieve the token
        // Future<String?> getUserTokenApi() async {
        //   final prefs = await SharedPreferences.getInstance();
        //   return prefs.getString('token');
        //
        // }
        //
        // String? tokena = await getUserTokenApi();
        //
        //
         print("my Token : $token");
        // print("my token $token ${token} $saveUserTokenApi(token) $getUserTokenApi()");



      } else if (temporaryToken != null) {
        //print("=====Else=====>>");
        await sendVerificationCode(Provider.of<SplashController>(Get.context!, listen: false).configModel!, SignUpModel(email: userInputData, phone: userInputData), type: Type, fromPage: fromPage);
      }
  ...
  Future<void> callFirebaseStoretiken (String phoneNumber, String vID) async {
    print("===1234==>>${vID}");
    ApiResponse apiResponse = await authServiceInterface.firebaseAuthTokenStore(userInput: phoneNumber, token: vID);
