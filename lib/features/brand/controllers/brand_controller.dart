...
class BrandController extends ChangeNotifier {
  ...
  Future<void> getBrandList(bool reload, {int offset = 1}) async {
    ...
    // Handle local cached data
    if (localData != null && offset == 1) {
      var brandList = jsonDecode(localData.response);

      if (brandList is List) {
        // If the data is a JSON array
        _brandModel = Brand(brands: brandList.map((e) => BrandModel.fromJson(e)).toList());
        _originalBrandList.addAll(_brandModel!.brands!);
        _brandList.addAll(_brandModel!.brands!);
      } else if (brandList is Map<String, dynamic>) {
    ...
    if (apiResponse.response?.statusCode == 200) {
      ...
      if (data is List) {
        // Handle JSON array from the API response
        _brandModel = Brand(brands: data.map((e) => BrandModel.fromJson(e)).toList());
        _originalBrandList.addAll(_brandModel!.brands!);
        _brandList.addAll(_brandModel!.brands!);
      } else if (data is Map<String, dynamic>) {
        // Handle JSON object from the API response
        if (offset == 1 && Brand.fromJson(data).brands != null) {
          _brandList.clear();
          _originalBrandList.clear();

          _brandModel = Brand.fromJson(data);
          data['brands'].forEach((brand) => _originalBrandList.add(BrandModel.fromJson(brand)));
          data['brands'].forEach((brand) => _brandList.add(BrandModel.fromJson(brand)));
        ...
        } else if (Brand.fromJson(data).brands != null && Brand.fromJson(data).brands!.isNotEmpty) {
          _brandModel = Brand.fromJson(data);
          data['brands'].forEach((brand) => _originalBrandList.add(BrandModel.fromJson(brand)));
          data['brands'].forEach((brand) => _brandList.add(BrandModel.fromJson(brand)));
        }
